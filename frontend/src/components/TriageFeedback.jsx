...
const TriageFeedback = ({ result, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white border border-[#e2e6ea] rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto animate-modalIn">
        {/* Verdict header */}
        <div className="px-6 py-5 flex items-center gap-4">
          <span
            className="w-10 h-10 rounded-full inline-flex items-center justify-center border-[3px] text-[#1a2332] shrink-0"
          ...
          <div className="min-w-0">
            <h2 className="text-lg sm:text-xl font-semibold text-[#1a2332]">
              {correct ? 'Correct classification' : 'Not quite'}
            </h2>
            <p className="text-sm text-[#57606a] mt-0.5">
              You classified as <span className="text-[#1a2332] font-medium">{selected_category}</span>
        ...
        <div className="px-6 py-5">
          {loading ? (
            <p className="text-sm text-[#57606a]">Loading triage review…</p>
          ) : review ? (
            <>
              {review.mitre && (
                <div className="mb-4">
              ...
              {review.what_is_it && (
                <div className="mb-4">
                  <span className="text-sm sm:text-base text-[#1a2332] font-medium">{review.what_is_it.title}</span>
                  <p className="text-[#1a2332] mt-2 leading-relaxed text-sm sm:text-base break-words">
                      ...
                      return (
                        <div key={i} className="flex gap-3">
                          <div className="relative w-3 flex-shrink-0 self-stretch">
                            {!isFirst && <div className="absolute left-1/2 -translate-x-1/2 w-px bg-[#d0d7de]" style={{ top: 0, bottom: '50%' }} />}
                            {!isLast && <div className="absolute left-1/2 -translate-x-1/2 w-px bg-[#d0d7de]" style={{ top: '50%', bottom: 0 }} />}
            ...
            <p className="text-sm text-[#1a2332]">Triage review not available for this scenario.</p>
          )}
        </div>
