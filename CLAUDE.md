...
- **Expressibility rule (ruled):** a scenario implying a correct response outside the seven-action vocabulary must be flagged in its scaffold; owner either reshapes the evidence or extends the vocabulary via a separate reviewed schema+engineering change. Never stretch an existing action's meaning. Unresolved expressibility blocks implementation: the scenario must not be marked `actions_reviewed`
