...
- **Scoring (see docs/action-scoring.md, the single reference):** `compute_action_score` consumes successful actions only; `no_op` and `failed_precondition` are score-neutral. Isolation graded on END-STATE at submission (released required containment = forfeited credit, nothing stacked; clean-host isolation collateral only while in effect); kill/delete/identity on OCCURRENCE. `release_host` absent from the answer-key enum: rollback control, never a gradable step. Taxonomy: successful unnecessary = collateral; failed_precondition = score-neutral but surfaced factually in the Response section (count + entries, no editorial label); no_op = score-neutral, unsurfaced; malformed/foreign/stale/wrong-kind = rejected, never logged, never scored
