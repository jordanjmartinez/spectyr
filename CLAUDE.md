...
- **Stage 3.9 CLOSED (2026-07-19).** 3.9A + 3.9B merged to `main`; the **Stage 3.9 baseline is merge commit `97e3292`**. Training is unreachable from the player-facing UI (the mode picker offers only Guided / SOC Queue / Hardcore); `GUIDED_MODES` retains the `"training"` key purely as **legacy compatibility** for the frozen 3.9A tests, and the internal `analyst` key backs the SOC Queue presentation rename (both implementation details). Four post-merge runtime checks passed (unassisted Guided; Practice Another world reset; SOC Queue cap + replacement lifecycle; Hardcore wrong-submit failure) with zero console errors. **Next: Stage 4 — the SIEM Investigation Workbench design contract (no-code deep dive, review artifact; no LCQL/multi-host implementation until approved).**
