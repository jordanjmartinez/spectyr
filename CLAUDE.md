...
│   │       ├── Reports.jsx          # Report management (table layout)
...
- CSS Grid layout with fixed column widths prevents layout shift when counts change
- Desktop: `grid-cols-[8rem_8rem_8rem_8rem_1fr]`, Mobile: `grid-cols-[5.5rem_5.5rem_5.5rem_5.5rem_1fr]`
- Counts always shown (Splunk-style): `Alerts (3)`, `Events (12)`, `Reports (2)`
...
- Alert Types and Total Alerts use side-by-side layout (chart left, legend right)
- Chart sizing: `w-36 h-36 sm:w-56 sm:h-56` (consistent across Grade ring and alert charts)
- Grade ring shows `"?"` placeholder when no data exists
- Total Alerts legend uses inline layout (number next to label, not stacked)
